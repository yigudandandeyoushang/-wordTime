  ...
  MobileArea.prototype = {
    ...
    bindEvent: function () {
      ...
      function popupArea(e) {
        ...
        //pc鼠标事件
        area_city.addEventListener('mousedown',gearMouseDown)
        area_city.addEventListener('mousemove', getMouseMove);
      }
      //初始化插件默认值
      ...
      function gearMouseDown(e) {
        ...
        clearInterval(target["int_" + target.id]);
        target["old_" + target.id] = e.clientY;
        target["o_t_" + target.id] = (new Date()).getTime();
        var top = target.getAttribute('top');
        if (top) {
          target["o_d_" + target.id] = parseFloat(top.replace(/em/g, ""));
      ...
      function getMouseMove(e) {
        ...
        target["new_" + target.id] = e.clientY;
